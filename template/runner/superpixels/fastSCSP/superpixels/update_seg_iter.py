#!/usr/bin/env python
"""
Created on Fri Sep  5 10:48:06 2014
...
class _UpdateSegIter(KernelThinWrapper):    
    ...
    def __call__(self,img_gpu,seg_gpu,border_gpu,
        ...
        if do_input_checks:
            # types
            for arg in [img_gpu,seg_gpu,border_subset_gpu, mu_s_gpu, mu_i_gpu]:
        ...
        for iter in range(nIters):
            for xmod3 in range(3):
                for ymod3 in range(3):  
                    ...
                    self._gpu_update_seg_subset(img_gpu, seg_gpu, border_gpu,
                        counts_gpu,  log_count_gpu, 
