...
__global__  void update_seg_subset(
    double* img, bool* img_isNaN, int* seg, const bool* border,
...
{   
    ...
    const bool pixel_isNaN = img_isNaN[idx];
    ...
    double resN = cal_posterior(isNvalid, N, calculate_cov, imgC, pt, log_counts, prior_weight, 
                mu_i_N, mu_s_N, J_i_N, J_s_N, logdet_Sigma_i,logdet_Sigma_s, i_std, s_std, pixel_isNaN);
    
    double resS = cal_posterior(isSvalid, S, calculate_cov, imgC, pt, log_counts, prior_weight, 
                mu_i_S, mu_s_S, J_i_S, J_s_S, logdet_Sigma_i,logdet_Sigma_s, i_std, s_std, pixel_isNaN);
    
    double resE = cal_posterior(isEvalid, E, calculate_cov, imgC, pt, log_counts, prior_weight, 
                mu_i_E, mu_s_E, J_i_E, J_s_E, logdet_Sigma_i,logdet_Sigma_s, i_std, s_std, pixel_isNaN);
    
    double resW = cal_posterior(isWvalid, W, calculate_cov, imgC, pt, log_counts, prior_weight,
                mu_i_W, mu_s_W, J_i_W, J_s_W, logdet_Sigma_i,logdet_Sigma_s, i_std, s_std, pixel_isNaN);
