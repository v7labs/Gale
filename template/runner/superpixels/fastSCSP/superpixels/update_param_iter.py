...
class _UpdateParamIter(KernelThinWrapper):    
    ...
    def __call__(self,img_gpu,seg_gpu,sp, 
                    calculate_s_cov=True, 
                    threads_per_block = int(1024/4),
                    nsuperpixel=None, 
                    do_input_checks=False): 
        # Part 1: input checks       
        if do_input_checks:
            for arg in [img_gpu,seg_gpu]:
        ...
        if calculate_s_cov:
            ...
            self._gpu_clear_fields(count, log_count, mu_i_h, mu_s_h, mu_i, mu_s, sigma_s_h,
                    dim_i, nsuperpixel,
                    grid=(num_block2,1,1), 
            ...
            self._gpu_sum_by_label (img_gpu, seg_gpu, 
                    count, mu_i_h, mu_s_h, sigma_s_h,
            ...
            self._gpu_calculate_mu_and_sigma(count, log_count, mu_i_h, mu_s_h, 
            ...
            self._gpu_clear_fields_2(count, mu_i_h, mu_s_h,
                    dim_i, nsuperpixel,
                    grid=(num_block2,1,1), 
                    block=(threads_per_block,1,1)
                   )         
            #calculate all the needed sums: count, mu_i_h, mu_s_h, parallelize over pixels
            self._gpu_sum_by_label_2(img_gpu, seg_gpu, 
                    count, mu_i_h, mu_s_h,
                    xdim, ydim, dim_i, nPts, 
                    grid=(num_block1,1,1), 
                    block=(threads_per_block,1,1)
            ...
            self._gpu_calculate_mu(count,mu_i_h, mu_s_h, mu_i, mu_s,
                    dim_i, nsuperpixel,
                    grid=(num_block2,1,1), 
                    block=(threads_per_block,1,1)
                    ) 
