#!/usr/bin/env python
"""
Created on Tue Sep 16 08:29:45 2014
...
from template.runner.superpixels.fastSCSP.superpixels.Superpixels import Superpixels

from template.runner.superpixels.fastSCSP.superpixels.init_seg import get_init_seg,random_permute_seg
from template.runner.superpixels.fastSCSP.superpixels.find_border_pixels import find_border_pixels
from template.runner.superpixels.fastSCSP.superpixels.rgb_to_lab import rgb_to_lab
from template.runner.superpixels.fastSCSP.superpixels.lab_to_rgb import lab_to_rgb
from template.runner.superpixels.fastSCSP.superpixels.get_cartoon import get_cartoon
from template.runner.superpixels.fastSCSP.superpixels.update_seg_iter import update_seg_iter as _update_seg_iter
from template.runner.superpixels.fastSCSP.superpixels.update_param_iter import update_param_iter as _update_param_iter
...
class SuperpixelsWrapper(object):
    ...
    def calc_seg(self, nEMIters, nItersInner=10, calc_s_cov=True, prior_weight=0.5, verbose=False):
        ...
        for i in range(nEMIters): 
            ...
            "(Hard) E step"
            self._calc_seg(img_gpu=self.img.gpu,seg_gpu=self.seg.gpu,
    ...
    def get_segmentation_bins(self, initial=False):
        """Return a mask where each pixel value represent the index of the superpixel it belong to"""
        return self.seg_ini if initial else self.seg.cpu

    def get_border_mask(self, initial=False):
        """Return a mask where each pixel value is a boolean denoting if it belong to a contour"""
        return self.border_ini if initial else self.border.cpu
