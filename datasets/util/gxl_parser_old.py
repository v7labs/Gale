...
class ParsedGxlDataset:
    def __init__(self, path_to_dataset, categorical_features=None, subset=None, remove_coordinates=False,
                 features_to_use=None, no_empty_graphs=False, center_coordinates=False):
        """
        ...
        for filename in self.all_file_names:
            try:
                try:
                    ...
                    logging.warning(f'{filename} does not appear in the dataset split files')

                g = ParsedGxlGraph(os.path.join(self.root_path, filename), subset, self.class_int_encoding[class_label],
                                   remove_coordinates=self.remove_coordinates, features_to_use=self.features_to_use, no_empty_graphs=self.no_empty_graphs)
                graphs.append(g)
            except InvalidFileException:
                logging.warning(f'File {filename} is invalid. Please verify that the file contains the expected attributes (id, edgeids, edgemode and nodes and edges, if expected)')
    ...
    def get_graphs(self):
        ...
        for filename in self.all_file_names:
            try:
                ...
                logging.warning(f'File {filename} is invalid. Please verify that the file contains the expected attributes (node, edge, id, edgeids and edgemode)')
                self.invalid_files.append(filename)
...
class ParsedGxlGraph:
    def __init__(self, path_to_gxl, subset, class_label, remove_coordinates=False, features_to_use=None, no_empty_graphs=False):
        """
    ...
    def verification(root, no_empty_graphs):
        ...
        if no_empty_graphs:
            logging.warning('Excluding empty graphs')
